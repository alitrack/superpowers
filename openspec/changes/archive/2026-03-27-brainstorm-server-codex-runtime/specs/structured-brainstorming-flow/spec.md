...
The system SHALL let the backend decide which question comes next so branching logic stays consistent across hosts and is not hardcoded into individual renderers, regardless of whether the session is running through app-server or exec fallback.
...
- **WHEN** browser and terminal hosts use the same backend runtime contract
...
#### Scenario: Artifact readiness is received
- **WHEN** the backend emits `artifact_ready`
- **THEN** the host transitions from question entry to result review mode and stops presenting another unanswered question
