...
if [ "$TURN_STATUS" -eq 2 ]; then
    print_skip_notice "$OUTPUT_DIR/turn1.json" "Haiku explicit skill test skipped during Turn 1"
    exit 2
fi
...
set -e
if [ "$TURN_STATUS" -eq 2 ]; then
    print_skip_notice "$OUTPUT_DIR/turn2.json" "Haiku explicit skill test skipped during Turn 2"
    exit 2
fi
if [ "$TURN_STATUS" -ne 0 ]; then
...
set -e
if [ "$TURN_STATUS" -eq 2 ]; then
    print_skip_notice "$OUTPUT_DIR/turn3.json" "Haiku explicit skill test skipped during Turn 3"
    exit 2
fi
if [ "$TURN_STATUS" -ne 0 ]; then
...
set -e
if [ "$TURN_STATUS" -eq 2 ]; then
    print_skip_notice "$OUTPUT_DIR/turn4.json" "Haiku explicit skill test skipped during Turn 4"
    exit 2
fi
if [ "$TURN_STATUS" -ne 0 ]; then
...
set -e
if [ "$TURN_STATUS" -eq 2 ]; then
    print_skip_notice "$FINAL_LOG" "Haiku explicit skill test skipped during Turn 5"
    exit 2
fi
if [ "$TURN_STATUS" -ne 0 ]; then
